import { generateTypeScript } from '@breakit/abl-mcp-contracts'
...
export default {
  ...
  handler: async ({ name, description, fields }: Record<string, unknown>) => {
    ...
    const content = generateTypeScript({ name: name as string, description: description as string | undefined, fields: f || [] })
